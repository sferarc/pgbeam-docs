...
- **Columns**: there is no column allowlist. To restrict what individual columns return, use [masking](/docs/masking) (redact, null, or hash a column's values in flight).
...
For columns that exist but should never leave the database in cleartext, reach for [masking](/docs/masking): the column stays usable for joins and grouping, but its values are hashed or redacted in flight.
