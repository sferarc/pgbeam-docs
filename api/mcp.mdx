...
<Callout type="info" title="Two different MCP servers">
  ...
  **query access to a database**, use the [hosted agent-database
  MCP](/docs/mcp), a separate endpoint with `query`, `validate_sql`,
  `list_tables`, `describe_table`, `explain`, and `schema_catalog` tools enforced
  by your policies, plus `search_docs` and `read_doc`.
</Callout>
