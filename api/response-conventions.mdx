...
- Sent on every attempt (including the first) so the server can deduplicate
  even if the initial request succeeds but the client loses the response
- GET and DELETE are naturally idempotent — no key needed
- The server caches idempotent responses for **24 hours**
- Transient errors (408, 429, 502, 503, 504) are **not** cached — the next
  retry executes the handler again
