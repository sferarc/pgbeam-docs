...
```json
{
  "projects": [{ "id": "prj_01j9x8y7z6w5v4u3t2s1r0q9p8" }],
  "next_page_token": "eyJpZCI6InByal8wMWo5eDh5N3o2In0"
}
```

### Follow the Link header instead

Every page that has a successor also carries an [RFC 8288](https://www.rfc-editor.org/rfc/rfc8288.html) `Link` header:

```http
Link: </v1/projects?page_size=20&page_token=eyJpZCI6…>; rel="next"
```
...
```ts
let url: string | null = "https://api.pgbeam.com/v1/projects?page_size=100";
while (url) {
  const res = await fetch(url, { headers });
  for (const project of (await res.json()).projects) handle(project);
...
Every error is an [RFC 9457](https://www.rfc-editor.org/rfc/rfc9457.html) problem detail, served as `application/problem+json`:

```json
{
  "type": "https://pgbeam.com/docs/api/errors/not-found",
  "title": "Not found",
  "status": 404,
  "detail": "Project not found",
  "instance": "/v1/projects/prj_abc123",
  "code": "NOT_FOUND",
...
| --- | --- | --- |
| `type` | `string` | URI identifying the condition; resolves to its entry in the error catalog |
| `title` | `string` | Short summary of the condition |
| `status` | `integer` | HTTP status, repeated so the document stands alone |
| `detail` | `string` | Explanation of this occurrence, written for a person |
| `instance` | `string` | Path of the request that failed |
| `code` | `string` | Machine-readable identity; the same identity as `type`, as a short token |
| `request_id` | `string` | Correlation id, also in the `X-Request-Id` header |
| `errors` | `array` | Field-level detail on a validation failure, each entry `{ field, detail }` |

### Error codes

Every code, what causes it and what to do about it is in the [error catalog](/docs/api/errors). Branch on `code`, not on `detail`: the detail is written for a person and is free to change, and two conditions can share a status. A `403` is either a permissions problem (`FORBIDDEN`) or a billing one (`PLAN_LIMIT_REACHED`), and those are resolved differently.
...
```http
GET /v1/projects/prj_abc123/webhooks/whk_xyz
ETag: "515a477090e052427bdb6093be3865ea"
Cache-Control: private, no-cache

GET /v1/projects/prj_abc123/webhooks/whk_xyz
If-None-Match: "515a477090e052427bdb6093be3865ea"
...
```http
PUT /v1/projects/prj_abc123/webhooks/whk_xyz
If-Match: "515a477090e052427bdb6093be3865ea"

HTTP/1.1 412 Precondition Failed
ETag: "aa5d8df00b533ffc227e9d74ffe17089"
{"error":{"code":"PRECONDITION_FAILED","message":"the resource has changed since the version this request was based on; re-read it and retry"}}
```
...
|  | Prevents |
| --- | --- |
| `Idempotency-Key` | A retry creating a **second** resource |
| `If-Match` | A write landing on a resource that is **no longer the one you read** |
...
```json
{
  "type": "https://pgbeam.com/docs/api/errors/idempotency-key-reused",
  "title": "Idempotency key reused with a different body",
  "status": 409,
  "detail": "Idempotency-Key was already used for a request with a different body; use a new key for a new request",
  "code": "IDEMPOTENCY_KEY_REUSED"
}
```

The code is what tells this apart from the other 409s. See the [error catalog](/docs/api/errors) for the full list.
