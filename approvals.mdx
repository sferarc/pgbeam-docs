...
A held request that no one acts on expires after the policy's expiry window. The statement is rejected, the agent is told, and the request closes. This keeps a forgotten request from holding an agent session open forever. Every approval, rejection, and expiry is written to the [audit log](/docs/audit-log) and can fire an [`approval_requested`](/docs/audit-export) webhook.

## Related

- [Read-only enforcement](/docs/read-only): block writes entirely instead of holding them.
- [Sandbox writes](/docs/sandbox-writes): let an agent write freely against a throwaway branch.
