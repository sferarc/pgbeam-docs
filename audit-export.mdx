...
Audit export pushes PgBeam events to your systems as they happen. Point it at a webhook endpoint and PgBeam delivers a signed JSON payload for each event. Point it at a SIEM and PgBeam formats the events the way that SIEM expects. The [audit log](/docs/audit-log) keeps the full history for querying; export is for getting events out in real time.
...
Each delivery is a JSON body with the event, the project, and the event-specific detail under `data`. The [webhook events](/docs/webhook-events) page documents the `data` fields for every event type.
