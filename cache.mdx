...
title: Caching
icon: Zap
description: How PgBeam caches query results at the edge, when it bypasses cache, and how to turn it on safely for your workload.
---
...
If you need cached data cleared sooner than its TTL, opt in to event-based invalidation. PgBeam listens on the `pgbeam_invalidate` channel on your upstream database and evicts cached entries for a table when it receives a notification naming that table. You emit those notifications from triggers you install on the tables you cache:
...
The `NOTIFY` payload is the table name (bare or schema-qualified). PgBeam evicts the cached entries that read from that table.
...
If a table changes often and must never be served stale, do not rely on invalidation timing. Either skip the cache for those reads with `/* @pgbeam:cache noCache */`, or give them a very short TTL (for example `/* @pgbeam:cache maxAge=1 */`) so the freshness window is bounded regardless of whether a notification arrives.
...
4. **Monitor cache hit rates**: Use the dashboard or [Query Insights](/docs/api) to see whether caching is actually helping.
