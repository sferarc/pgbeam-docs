...
The operation ids and shapes come from the [OpenAPI spec](/docs/api). The typical flow is `search_endpoints` → `describe_endpoint` → `call_endpoint`. Both transports expose the same three meta-tools.
