...
Display usage metrics for the linked project over a date range. Shows daily
breakdowns of total queries, cache hits, and data transferred by region.
Defaults to the current calendar month if no date range is specified.
...
columns: Day, Region, Queries, Cache Hits, and Data. With `--json`, returns the
