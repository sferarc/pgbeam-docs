...
3. PgBeam parses the statement and checks it: access mode (read-only or read-write), table allowlists, and statement type.
...
- A **scoped connection string** for any PostgreSQL driver, ORM, or framework. See [Connection string](/docs/connection-string).
- A **hosted MCP endpoint** for Claude Code, Cursor, or any MCP client. See [Hosted MCP](/docs/mcp).
...
- Binary-format result columns are masked to `NULL`; text-format columns get a redaction token. See [Masking](/docs/masking) for the semantics.

## Next

- [How PgBeam works](/architecture): the long-form mechanism, including why the policy lives in the wire and how pooling, caching, and nearest-region routing keep the guard in the query path.
