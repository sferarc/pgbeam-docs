...
description: Query-level analytics for a project. Top query shapes by call count, with cache hit rate and latency for each, plus aggregate cache and latency summaries.
---

Insights shows what your project's traffic actually looks like. It groups statements by normalized shape, ranks them by call count, and reports the cache hit rate and latency for each shape. Alongside the per-query breakdown it gives you two aggregate summaries: overall cache performance and overall latency for the window you pick. Use it to find the queries worth caching, spot a shape whose latency has crept up, and see how much the [cache](/docs/cache) is saving you.
...
- A shape with a high `total_count` and a low `total_cache_hits` is a caching opportunity. Add a [cache annotation](/docs/cache) and watch the hit rate climb on the next scan.
...
- [Anomaly detection](/docs/anomaly-detection): alerts when a credential's traffic drifts from its baseline.
