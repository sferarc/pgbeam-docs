...
Masking then works on the view exactly as it does on a table, guard included. The policy editor flags this for you: when a relation you name resolves to a view, it says so above the field. Allowlists and [row filters](/docs/policies) carry the same limit, so a view over a row-filtered table serves every row.
...
Masking and [allowlists](/docs/allowlists) work together. Allowlisting decides which relations and columns an agent may reach at all; masking decides what the allowed column's value looks like when it comes back.
