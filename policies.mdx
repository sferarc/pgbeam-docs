...
title: Policies
icon: ShieldCheck
description: A policy profile is the named bundle of rules PgBeam enforces for an agent credential. Access mode, allowlists, masking, budgets, and timeouts.
---
...
Policies apply to agent credentials and to human credentials alike. Every rule below works the same whether an AI agent or a person runs the statement. See [human and passthrough connections](#human-and-passthrough-connections) for applying policies beyond per-agent credentials.
...
For testing against a local or dev database before anything reaches PgBeam at all, the `pgbeam-dev` tool evaluates a policy file offline and can run a local enforcing gateway. See [local policy testing](/docs/local-testing).
...
A policy can bind to more than a per-agent credential. Every credential carries a `principal_type`: `agent` for an AI agent's credential, `human` for a person's. The rules are identical; the field records who is connecting so the [audit log](/docs/audit-log) and [anomaly baselines](/docs/anomaly-detection) can tell agents and people apart.
