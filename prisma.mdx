...
Or switch the project to **session** pool mode if you rely on session-level features. See [Connection Pooling](/docs/pooling).
...
For standard Prisma Client queries (`findMany`, `findUnique`, etc.), PgBeam automatically tracks the generated SQL shapes. Enable caching for these shapes through [Cache Rules](/docs/cache) in the dashboard. No code changes needed.
...
See [Read Replicas](/docs/replicas) for details on replica setup and routing behavior.
