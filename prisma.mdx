...
<Steps>
  ...
  <Step>
    ...
    ```bash title=".env"
    DATABASE_URL="postgresql://user:pass@abc.proxy.pgbeam.app:5432/mydb?connection_limit=5"
    ```
...
<Callout type="warn" title="Transaction mode + Prisma prepared statements">
...
```bash title=".env"
DATABASE_URL="postgresql://user:pass@abc.proxy.pgbeam.app:5432/mydb?connection_limit=5&pgbouncer=true"
```

Or switch the project to **session** pool mode if you rely on session-level
features. See [Connection Pooling](/docs/pooling).
...
// Check server logs for: NOTICE: pgbeam:cache hit age=12s ttl=1m0s swr=30s
```
