...
<Callout type="warn" title="One required step: point the driver at PgBeam">
...
```ts title="neon-config.ts (import once at startup)"
...
These two lines are the only addition. Your query call sites do not change.
...
`json` / `jsonb` are parsed into objects, and arrays into JS arrays, exactly as
...
| WebSocket `Pool` / `Client` connects then hangs, no error | Default `pipelineConnect: "password"` pipelines a cleartext password, but PgBeam uses SCRAM | Set `neonConfig.pipelineConnect = false`. If you only need one-shot queries, use the HTTP `neon()` path instead |
