...
1. **Verify the hostname.** It should be `<project-id>.proxy.pgbeam.app` on port `5432`. Check for typos.
...
2. **Check the hostname.** The certificate matches `*.proxy.pgbeam.app`. If you are using an IP address or a hostname that does not match, TLS validation will fail.
...
This is expected behavior. See [Resilience](/docs/resilience) for details on scale-to-zero.
...
4. Contact support at [support@pgbeam.com](mailto:support@pgbeam.com) with the debug output, SQLSTATE, and steps to reproduce
